...
class ESM2(nn.Module):
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            #print(f"Layer output type: {type(layer_output)}")
            #print(f"Layer output length: {len(layer_output) if isinstance(layer_output, tuple) else 'Not a tuple'}")
    ...
    def predict_contacts(self, tokens: torch.Tensor) -> torch.Tensor:
        """
